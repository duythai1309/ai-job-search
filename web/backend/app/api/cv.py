...
from app.models.schemas import CVCreate, CVUpdate, ApplySuggestionRequest, GenerateCoverLetterRequest, TailorCvRequest, UserInfo
...
async def analyze_cv(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"AI phân tích CV lỗi: {e}")

    client.table("cv_suggestions").delete().eq("cv_id", str(cv_id)).eq("user_id", str(user.id)).execute()

    _allowed = ("section", "suggestion_type", "original_text", "suggested_text", "reason")
    records = []
    for s in suggestions:
        if not isinstance(s, dict):
            continue
        rec = {k: s.get(k) for k in _allowed}
        rec.update({
            "cv_id": str(cv_id),
            "user_id": str(user.id),
...
@router.post("/tailor")
async def tailor_cv(body: TailorCvRequest, user: UserInfo = Depends(get_current_user)):
    """Analyse the user's profile (or a chosen CV) against a job and return a
    per-line, job-tailored analysis the frontend renders for inline editing."""
    client = get_service_client()
    ...
    if body.cv_id:
        cv_resp = client.table("cvs").select("*").eq("id", str(body.cv_id)).eq("user_id", str(user.id)).execute()
        if cv_resp.data:
            user_profile["current_cv"] = cv_resp.data[0]

    try:
        result = await gemini.tailor_cv_to_job(
        ...
        raise HTTPException(status_code=500, detail=f"Lỗi tinh chỉnh CV: {str(e)}")
