...
from app.models.schemas import CrawlRequest, FitEvaluation, JobPostingOut, UserInfo
...
async def search_jobs(
    ...
    """Read from the DB only — fast and free. Use /jobs/crawl to refresh data."""
    source_list = [s.strip() for s in sources.split(",") if s.strip()] or None
    jobs = job_scraper.query_db(q, location, source_list, page, limit)
    return {"jobs": jobs, "total": len(jobs), "page": page}


@router.post("/crawl")
async def crawl_jobs(body: CrawlRequest, user: UserInfo = Depends(get_current_user)):
    """Scrape live (Firecrawl) and store into the DB. Triggered by the manual
    'refresh' button (with a query) or the periodic cron (default queries)."""
    sources = [s.strip() for s in body.sources if s.strip()] if body.sources else None
    queries = [body.q.strip()] if (body.q and body.q.strip()) else job_scraper.DEFAULT_CRAWL_QUERIES
    return await job_scraper.crawl_and_store(queries, sources, body.location or "")
...
async def evaluate_job_fit(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"AI đánh giá độ phù hợp lỗi: {e}")
