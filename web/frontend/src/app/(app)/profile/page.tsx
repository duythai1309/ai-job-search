...
export default function ProfilePage() {
  ...
  const usedSkillCats = SKILL_CATEGORIES.filter((c) => skills.some((s) => s.category === c.id));
  const showSkillCatLabels = usedSkillCats.length > 1;
    ...
    <div className="px-8 py-10 max-w-3xl">
      ...
      {activeTab === "skills" && (
        <div className="space-y-6">
          {/* Add a new skill — its own card so it reads as an input area, not a tag list */}
          <div className="rounded-2xl p-5 sm:p-6 border border-slate-200 bg-white">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <input
                ...
                placeholder="Nhập tên kỹ năng, ví dụ Python…"
              ...
              <button
                ...
                className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-sm font-medium disabled:opacity-50 transition-colors cursor-pointer whitespace-nowrap"
              >
                Thêm
              </button>
            </div>
          ...
          {/* Skills the user already has — framed clearly as owned, with an obvious delete */}
          <div className="rounded-2xl p-5 sm:p-6 border border-slate-200 bg-white">
            {skills.length === 0 ? (
              <div className="text-center py-10">
                <p className="text-sm font-medium text-slate-600">Chưa có kỹ năng nào</p>
                <p className="text-slate-400 text-sm mt-1">
                  Thêm kỹ năng ở trên, hoặc tải CV lên để AI tự trích kỹ năng cho bạn.
                </p>
              </div>
            ) : (
              <>
                <div className="flex items-center gap-2 mb-1">
                  <h2 className="text-sm font-semibold text-slate-900">Kỹ năng của bạn</h2>
                  <span className="text-xs font-semibold text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full">
                    {skills.length}
                  </span>
                </div>
                <p className="text-xs text-slate-400 mb-5 flex items-center gap-1 flex-wrap">
                  Đã lưu trong hồ sơ (AI trích từ CV của bạn). Di chuột vào một thẻ rồi bấm
                  <span className="inline-flex items-center justify-center w-4 h-4 rounded bg-slate-100 text-slate-500">
                    <X className="w-3 h-3" />
                  </span>
                  để xóa.
                </p>
                      ...
                      <div key={cat.id}>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {catSkills.map((skill) => (
                            <span
                              key={skill.id}
                              className="group inline-flex items-center gap-1.5 bg-slate-100 border border-slate-200 text-slate-800 pl-3 pr-1.5 py-1.5 rounded-lg text-sm"
                            >
                              ...
                              <button
                                ...
                                title="Xóa kỹ năng"
                                className="ml-0.5 w-5 h-5 flex items-center justify-center rounded-md text-slate-400 hover:bg-red-100 hover:text-red-600 transition-colors cursor-pointer"
                              >
